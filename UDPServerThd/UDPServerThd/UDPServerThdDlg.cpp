// UDPServerThdDlg.cpp: 대화상자와 ARQ 프로토콜 동작을 구현한 파일입니다.
#include "pch.h"
#include "framework.h"
...
BOOL InjectFrameError(Frame& frame, int errorType, CEdit& edit) // 선택한 오류를 다음 송신 Frame에 1회 주입합니다.
{
	...
	case 1: // checksum 오류를 주입하는 경우입니다.
		if (frame.payload_len > 0)
			...
			frame.checksum ^= 0x1;
		log.Format(_T("[주입] Checksum 오류 (checksum 미재계산) seq=%d\r\n"), frame.seq_num); // 화면에 표시할 로그 문자열을 구성합니다.
		AppendEditText(edit, log); // Edit Control 끝에 로그 문자열을 추가하는 함수입니다.
		return FALSE;
...
UINT TXThread(LPVOID arg) // 송신 큐에서 Frame을 꺼내 보내고 timeout 시 재전송하는 스레드입니다.
{
	...
	while (pArg->Thread_run) // 스레드 실행 여부를 나타내는 플래그입니다.
	{
		...
		if (hasFrame)
		{
			...
			if (pDlg->m_hSocket != INVALID_SOCKET && !pDlg->m_clientAddr.IsEmpty()) // 처리할 데이터가 없는지 확인합니다.
			{
				SOCKADDR_IN client_addr;
				memset(&client_addr, 0, sizeof(client_addr)); // 버퍼를 0으로 초기화합니다.
				client_addr.sin_family = AF_INET;
				client_addr.sin_port = htons(pDlg->m_clientPort);
				InetPton(AF_INET, pDlg->m_clientAddr, &client_addr.sin_addr);
				...
				{
					int sentBytes = sendto(pDlg->m_hSocket, (char*)&frame, sizeof(Frame), 0, (SOCKADDR*)&client_addr, sizeof(client_addr)); // Frame 구조체 전체를 UDP 패킷으로 전송합니다.
...
BEGIN_MESSAGE_MAP(CUDPServerThdDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_SEND, &CUDPServerThdDlg::OnBnClickedSend) // UI 이벤트와 처리 함수를 연결합니다.
	ON_BN_CLICKED(IDC_CLOSE, &CUDPServerThdDlg::OnBnClickedClose) // UI 이벤트와 처리 함수를 연결합니다.
	ON_BN_CLICKED(IDC_CORRUPT_NEXT, &CUDPServerThdDlg::OnBnClickedCorruptNext) // UI 이벤트와 처리 함수를 연결합니다.
	ON_BN_CLICKED(IDC_VERBOSE_LOG, &CUDPServerThdDlg::OnBnClickedVerboseLog) // UI 이벤트와 처리 함수를 연결합니다.
	ON_EN_CHANGE(IDC_EDIT1, &CUDPServerThdDlg::OnEnChangeEdit1) // UI 이벤트와 처리 함수를 연결합니다.
END_MESSAGE_MAP()
...
BOOL CUDPServerThdDlg::OnInitDialog()
{
	...
	if (m_hSocket != INVALID_SOCKET && bind(m_hSocket, (SOCKADDR*)&server_addr, sizeof(server_addr)) != SOCKET_ERROR)
	{
	...
	AfxMessageBox(_T("UDP 서버 소켓 생성 실패")); // UDP 소켓 생성 실패를 사용자에게 알립니다.
...
void CUDPServerThdDlg::ProcessReceive() // UDP Frame을 수신한 뒤 크기, checksum, seq/ACK, reassembly 순서로 검증합니다.
{
	Frame frame; // 처리할 Frame을 임시로 담는 변수입니다.
	CString PeerAddr; // 화면 출력에 사용할 문자열 변수입니다.
	UINT PeerPort;
	int nbytes;
	...
	TCHAR addrText[32];
	InetNtop(AF_INET, &peer_addr.sin_addr, addrText, 32);
	PeerAddr = addrText;
	PeerPort = ntohs(peer_addr.sin_port);
	m_clientAddr = PeerAddr;
	m_clientPort = PeerPort;
...
void CUDPServerThdDlg::OnBnClickedSend() // Send 버튼 클릭 시 입력 메시지를 Frame으로 나누어 송신 큐에 넣습니다.
{
	...
	if (m_clientAddr.IsEmpty()) // 처리할 데이터가 없는지 확인합니다.
	{
		AppendEditText(m_packet_log_edit, _T("[전송 불가] 클라이언트가 아직 접속하지 않았습니다. 클라이언트가 먼저 메시지를 보낸 뒤 전송하세요.\r\n")); // Edit Control 끝에 로그 문자열을 추가하는 함수입니다.
...
void CUDPServerThdDlg::OnBnClickedCorruptNext() // Corrupt 버튼 클릭 시 다음 Frame에 적용할 오류 주입을 예약합니다.
{
...
void CUDPServerThdDlg::OnBnClickedVerboseLog() // 상세 로그와 요약 로그 모드를 전환합니다.
{
...
void CUDPServerThdDlg::OnEnChangeEdit1() // 입력창 내용이 256Byte를 넘지 않도록 제한합니다.
{
...
void CUDPServerThdDlg::OnBnClickedClose() // 스레드를 종료하고 UDP 소켓을 닫습니다.
{
